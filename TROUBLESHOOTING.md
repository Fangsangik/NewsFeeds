...
## 2026-08-26~27 — 인스타형 대규모 확장(R1~R3) 중 발견/해결한 이슈

### T7) JPQL CASE로 엔티티를 직접 반환하면 500 (SemanticException)
- **증상**: 팔로워/팔로잉 목록(`/friends/members/{id}`)이 500. 로그에 `CaseSearchedExpression`.
- **원인**: `select case when ... then f.receiver else f.sender end`처럼 CASE가 **엔티티**를 반환하면 Hibernate 6가 SQL AST로 변환 못 함.
- **수정**: CASE는 **스칼라 id만** 뽑고(`then f.receiver.id else f.sender.id`), 서비스에서 `memberRepository.findAllById(ids)`로 엔티티 로드. (같은 패턴을 친구 목록·팔로워 목록에 적용)

### T8) `NOT IN (:empty)` 문법 오류 방지 (차단 필터)
- **맥락**: 팔로잉 피드에서 차단 회원 제외(`AND f.member.id NOT IN :blocked`).
- **문제**: 차단이 없어 `:blocked`가 빈 리스트면 `NOT IN ()`로 SQL 문법 오류.
- **수정**: 서비스에서 빈 리스트일 때 **sentinel `-1L`** 하나를 넣어 항상 비어있지 않게 전달.

### T9) 앱 재빌드 직후 요청이 500/실패로 보임 (기동 중)
- **증상**: `docker compose up -d --build app` 직후 새 엔드포인트가 잠깐 500, 브라우저 STOMP 연결 실패.
- **원인**: 컨테이너 스왑 중 스프링 컨텍스트 미기동 상태에서 요청이 들어옴(코드 문제 아님).
- **대응**: 검증은 **엔드포인트가 200/401을 안정적으로 줄 때까지 폴링 후** 실행. (헬스 신호 확인 전 테스트 금지)

### T10) 단일 세션 정책이 브라우저 STOMP 검증을 방해
- **증상**: 스크립트에서 여러 계정을 재로그인하며 STOMP 읽음 푸시를 검증하면 `connected:false`/수신 0건.
- **원인**: 로그인 시 `jwt_token`을 회원당 1개로 강제(단일 세션) → 나중 로그인이 앞 토큰(진행 중 STOMP)을 무효화.
- **대응**: (1) 서버 로그로 `convertAndSendToUser` 발행을 직접 확인, (2) 필요 시 `DELETE FROM jwt_token`으로 다중 토큰 공존 후 검증. 실제 앱은 로그인당 STOMP를 유지하므로 정상 동작.
- **트레이드오프 메모**: 데모/포폴에선 편의를 위해 다중 세션 허용(정책 완화)도 고려 가능.

### T11) 자기 글 답글은 답글 알림이 생략됨 (의도된 동작)
- **관찰**: 내 게시물의 내 댓글에 남이 답글 → '게시물 댓글' 알림만 오고 '답글' 알림은 안 옴.
- **원인**: 부모 댓글 작성자 == 게시물 작성자면 **중복 알림 방지**로 답글 알림을 생략.
- **결론**: 버그 아님. 부모 댓글 작성자 ≠ 게시물 작성자일 때만 별도 '답글' 알림(검증: Carol→Bob 댓글 답글 → Bob에게 도착).

### T12) 브라우저 자동화 `left_click_drag`가 스와이프로 인식 안 됨
- **증상**: 캐러셀 드래그 테스트 시 이미지가 안 넘어감.
- **원인**: 자동화 드래그가 `mousedown`/`mouseup` 좌표를 스와이프 핸들러 기대와 다르게 전달.
- **대응**: 실제 스와이프 로직은 정상. JS로 `MouseEvent(mousedown x=400 → mouseup x=180)`를 디스패치해 1/3→2/3→3/3 전환 확인.
